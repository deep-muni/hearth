...
func (d *Database) ensureIndexes(ctx context.Context) error {
	...
	helpersCol := d.DB.Collection("helpers")
	_, err := helpersCol.Indexes().CreateOne(idxCtx, mongo.IndexModel{
	...
	attendanceCol := d.DB.Collection("attendance")
	_, err = attendanceCol.Indexes().CreateOne(idxCtx, mongo.IndexModel{
		Keys: bson.D{
			{Key: "helperId", Value: 1},
			{Key: "date", Value: 1},
		},
		Options: options.Index().SetUnique(true),
	})
	...
	adjustmentsCol := d.DB.Collection("adjustments")
	_, err = adjustmentsCol.Indexes().CreateOne(idxCtx, mongo.IndexModel{
		Keys: bson.D{
			{Key: "helperId", Value: 1},
			{Key: "month", Value: 1},
		},
