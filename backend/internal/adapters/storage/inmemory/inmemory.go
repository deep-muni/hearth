...
func (s *Storage) Backup() ports.BackupRepository {
	return &BackupRepo{s: s}
}

var _ ports.BackupRepository = (*BackupRepo)(nil)

func (r *BackupRepo) ExportAll(_ context.Context) (*domain.BackupData, error) {
	r.s.mu.RLock()
	defer r.s.mu.RUnlock()
...
func (r *BackupRepo) ImportAll(_ context.Context, data domain.BackupData) error {
	r.s.mu.Lock()
	defer r.s.mu.Unlock()

	r.s.helpers = make(map[string]domain.HouseHelp)
	r.s.attendance = make(map[string]domain.AttendanceRecord)
	r.s.adjustments = make(map[string]domain.MonthlyAdjustment)
	...
	for _, a := range data.Attendance {
		key := a.HelperID + "_" + a.Date
		r.s.attendance[key] = a
	}
